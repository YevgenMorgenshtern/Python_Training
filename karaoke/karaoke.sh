...
if [ ! -f "$WORKDIR/instrumental.mp3" ] && [ ! -f "$WORKDIR/instrumental.wav" ]; then
  echo ""
  echo "  Используй UVR5 (Ultimate Vocal Remover) для удаления вокала."
  echo "  Скачать: https://github.com/Anjok07/ultimatevocalremovergui/releases"
  echo ""
  echo "  Входной файл:  👉  $(pwd)/$WORKDIR/song.mp3"
  echo "  Выходная папка: $(pwd)/$WORKDIR/"
  echo ""
  echo "  ┌─ Рекомендуемые модели (по качеству) ──────────────────────┐"
  echo "  │                                                             │"
  echo "  │  Два прохода для максимального качества:                   │"
  echo "  │    1й проход: MDX-Net → UVR-MDX-NET Karaoke               │"
  echo "  │    2й проход: VR Arch → 5_HP-Karaoke-UVR                  │"
  echo "  │    (результат 1го прохода подать на вход 2му)              │"
  echo "  │                                                             │"
  echo "  │  Один проход (быстрее):                                    │"
  echo "  │    MDX-Net → UVR-MDX-NET Karaoke                          │"
  echo "  │    MDX-Net → UVR-MDX-NET Inst HQ 2                        │"
  echo "  │    MDX-Net → kuielab_a_vocals                             │"
  echo "  │                                                             │"
  echo "  │  Настройки UVR:                                            │"
  echo "  │    Format:           WAV                                   │"
  echo "  │    Segment Size:     256                                   │"
  echo "  │    Overlap:          0.75                                  │"
  echo "  │    GPU Conversion:   ✅ включить                           │"
  echo "  │    Instrumental Only: ✅ включить                          │"
  echo "  └─────────────────────────────────────────────────────────────┘"
  echo ""
  echo "  Сохрани результат как:"
  echo "    👉  $(pwd)/$WORKDIR/instrumental.wav"
  echo ""
  read -r -p "  Готово? Нажми Enter чтобы продолжить... "

  if [ ! -f "$WORKDIR/instrumental.mp3" ] && [ ! -f "$WORKDIR/instrumental.wav" ]; then
    echo "❌ Файл $WORKDIR/instrumental.wav не найден. Положи его туда и запусти скрипт снова."
    exit 1
  fi
fi
